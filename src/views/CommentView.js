...
export default function CommentView({ ...props }) {
  ...
  return (
    <Layout>
      <Heading>
        {isPositiveRating ? POSITIVE_RATING_HEADING : NEGATIVE_RATING_HEADING}
      </Heading>
      <Subheading>
        Please describe your experience with the MongoDB Documentation.
      </Subheading>
      <CommentTextArea rows={8} placeholder="Tell us more." />
      <EmailInput placeholder="Email Address (optional)" />
      <Footer>
        <Button onClick={() => submitComment()}>
          {isSupportRequest ? "Continue for Support" : "Send"}
        </Button>
      </Footer>
    </Layout>
  );
